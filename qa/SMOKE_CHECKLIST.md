...
**Last updated:** v0.7.13 — Onboarding restored (#331 — P0): patch 003-server-py-onboarding-redirect wires `should_redirect_to_setup` into upstream's do_GET + _handle_write. The regression had been live since v0.6.0/v0.7.0 — caught by a 6-perspective retrospective today, not by CI or smoke. v0.7.14+ adds the process meta-fixes (required-smoke-CI, SMOKE_LOG.md, etc.) that prevent recurrence. Verification methodology rule unchanged: run against released `:stable` (or candidate `:latest` for pre-tag), not against a local `docker build` from source. The DMG-equivalent install command is the canonical path:
