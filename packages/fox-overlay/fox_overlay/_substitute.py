"""Shared anchor-based source substitution for Fox overlay patches.

Previously this lived in two copies — one in ``fox_overlay/webui_patches/
_helpers.py``, one in ``fox_overlay/agent_plugins/fox_overlay_plugin/
monkey_patches/_helpers.py`` — that had already drifted once (the webui
copy added ``substitute_method``; the agent copy didn't). The v0.7.5
cleanup pulled both into this single module; the two ``_helpers.py``
files are now thin re-export shims, preserved so existing imports
(``from ._helpers import substitute_function``) keep working.

## Pattern

Each monkey-patch module defines its target upstream function and a
list of ``(old, new)`` substitutions where ``old`` is an EXACT fragment
of the upstream function source. :func:`substitute_function` reads the
upstream source via :func:`inspect.getsource`, applies the substitutions
sequentially, and recompiles the result in the upstream module's
namespace so it can be assigned back as the new function.

If any ``old`` anchor is missing or appears more than once, an
``AssertionError`` is raised at import time — the container fails to
boot with a precise error pointing at the upstream drift. The nightly
``upstream-watch.yml`` CI catches this before users do.
...
## Note on the wrap-and-splice alternative

For *post-call mutations* of an upstream function's return value (vs
*in-function modifications*), prefer wrap-and-splice (replace the
module attribute with a closure that calls the original) over
``substitute_function``. Anchors deep inside large functions are
fragile against upstream refactors; wrap depends only on the function
signature + return-shape. First example: ``webui_patches/config.py
_wrap_get_available_models`` (v0.7.4 / #303).
"""
import inspect
import logging
import textwrap
...
_log = logging.getLogger("fox_overlay.substitute")
...
def substitute_function(
    ...
    body is indented under the class def), use :func:`substitute_method`
    instead — it dedents AFTER anchor matching is complete.
    """
...
def substitute_method(
    ...
    The "dedent normalizes blank lines, breaking anchors" warning that
    applies to :func:`substitute_function` does NOT apply here — dedent
    runs AFTER anchor matching is complete, so any whitespace
    normalization can't break anchors.
    ...
    for idx, (old, new) in enumerate(substitutions):
        ...
        if count != 1:
            raise AssertionError(
                "[fox-overlay] patch %s.%s.%s substitution #%d: "
                "anchor expected EXACTLY ONCE, found %dx. Upstream may have "
