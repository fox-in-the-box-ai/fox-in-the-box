"""Anchor-based source substitution helper for agent-side monkey-patches.
...
_log = logging.getLogger("fox_overlay.monkey_patch")
...
def substitute_function(
    ...
    or symbols not imported in the upstream module (e.g., a ``traceback``
    module the Fox patch wants to call but upstream doesn't import).
    ...
    for idx, (old, new) in enumerate(substitutions):
        ...
        if count != 1:
            raise AssertionError(
                ...
                "fox-overlay monkey-patch module. Anchor:\n%s"
