...
**Last updated:** v0.7.10 — Mobile avatar swap (#299): below 768px viewport, titlebar shows Fox avatar photo instead of upstream Hermes caduceus. CSS-only + a Playwright mobile-viewport spec that catches the three regression classes (load order, missing asset, upstream selector rename). Verification methodology rule unchanged: run against released `:stable` (or candidate `:latest` for pre-tag), not against a local `docker build` from source. The DMG-equivalent install command is the canonical path:
