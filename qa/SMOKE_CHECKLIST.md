...
**Last updated:** v0.7.12 — Tailscale auth-link sticky (#146 — pending Safari verification): server-side `last_auth_url` fallback in `/api/tailscale/up/poll` so transient empty values during awaiting-auth no longer cause Safari to lose the rendered link. Server-side-only MVP; client-side tile rebuild deferred to v0.7.13 if needed. Verification methodology rule unchanged: run against released `:stable` (or candidate `:latest` for pre-tag), not against a local `docker build` from source. The DMG-equivalent install command is the canonical path:
