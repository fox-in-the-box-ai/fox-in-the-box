...
**Last updated:** v0.6.2 — Mac launcher race fix (#271) + stream-error-retry polish (#267) + providers.py overlay retirement (#269). Verification methodology rule unchanged: run against released `:stable` (or candidate `:latest` for pre-tag), not against a local `docker build` from source. The DMG-equivalent install command is the canonical path:
