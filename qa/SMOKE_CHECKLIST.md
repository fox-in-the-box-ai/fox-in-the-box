...
**Last updated:** v0.6.0 — upstream-separation migration: virgin-upstream submodules + fox-overlay package + check-overlay-basis CI gate + nightly upstream-watch. Verification methodology rule unchanged: run against released `:stable` (or candidate `:latest` for pre-tag), not against a local `docker build` from source. The DMG-equivalent install command is the canonical path:
