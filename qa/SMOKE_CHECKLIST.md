...
**Last updated:** v0.7.4 — Local Ollama picker fix (#303 symptoms 1+2; wrap-and-splice in `fox_overlay/webui_patches/config.py`). Verification methodology rule unchanged: run against released `:stable` (or candidate `:latest` for pre-tag), not against a local `docker build` from source. The DMG-equivalent install command is the canonical path:
