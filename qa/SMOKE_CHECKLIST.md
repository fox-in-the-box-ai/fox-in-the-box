...
**Last updated:** v0.7.7 — Docs audit (#310 closed) + Playwright Phase 0 infrastructure (#264 closed). No code-behavior change to the running app; the kind of release that pays back over the entire stabilization cycle. Verification methodology rule unchanged: run against released `:stable` (or candidate `:latest` for pre-tag), not against a local `docker build` from source. The DMG-equivalent install command is the canonical path:
