...
// UNSKIP IN v0.7.20 — same chicken-and-egg pattern as wizard-renders.spec.ts
// v0.7.13 redirect block had (skipped until v0.7.17 shipped the fix to :stable).
// When v0.7.20 lands with the chat-auto-preselect fix, drop the .skip suffix
// AND remove the no-providers branch's "Model not selected" expectation if
// the fix uses different empty-state copy — check the v0.7.20 PR diff against
// static/ui.js:populateModelDropdown() before unskipping.
test.describe.skip('Phase 1 — #344 chat auto-preselect (v0.7.20+)', () => {
...
// ── Phase 1 — #278 Ollama dedup across groups (v0.7.20+, SKIPPED) ──────────
// UNSKIP IN v0.7.20 — the dedup fix belongs in
// fox_overlay/webui_patches/config.py:_splice_ollama_group(). When that
// lands, drop the .skip suffix. If the fix instead lands in upstream's
// _build_available_models_uncached() or a third location, update the
// failure-mode comment to point at the right file.
test.describe.skip('Phase 1 — #278 Ollama dedup across groups (v0.7.20+)', () => {
