...
def test_post_set_hostname_ok(fresh_dispatch_and_module, monkeypatch):
    ...
    monkeypatch.setattr(
        m,
        "handle_set_hostname",
        lambda h, body: {"ok": True, "hostname": body["hostname"]},
    )
    from urllib.parse import urlparse
    ...
    assert h.responses[-1] == {
        "status": 200,
        "payload": {"ok": True, "hostname": "fox-prod"},
    }
...
def test_post_set_hostname_failure_status_400(fresh_dispatch_and_module, monkeypatch):
    d, m = fresh_dispatch_and_module
    monkeypatch.setattr(
        m, "handle_set_hostname", lambda h, body: {"ok": False, "error": "invalid"}
    )
    from urllib.parse import urlparse
...
def test_post_dismiss_prompt_ok(fresh_dispatch_and_module, monkeypatch):
    ...
    monkeypatch.setattr(
        m,
        "handle_dismiss_hostname_prompt",
        lambda h, body: {"ok": True, "prompted": True},
    )
    from urllib.parse import urlparse
