...
- Floored `setuptools` to `>=78.1.1` in the container image to remediate CVE-2025-47273. (#897)
- The release pipeline now fails the release when the shipped container image carries a fixable CRITICAL/HIGH vulnerability: a Trivy scan of the release image (by immutable digest, fixable-only) now gates image promotion, so a vulnerable image can never become `:stable` or a `vX.Y.Z` tag. (#897)
...
- Releases carry their SBOM again: sbom.yml is invoked directly from the release pipeline — GitHub never fires `release: published` for workflow-published releases, so every release after v0.7.55 silently shipped without one; tag-targeted backfill supported, with a fail-loud guard against minting a release for a typo'd tag
- The apt publish job's stale `continue-on-error` (cited "#539 secrets not yet configured", false since the v0.7.59 publish) was removed — a real apt failure now reds the release run
- The nightly Playwright full suite got a budget sized to today's 57-test suite (three shards timed out invisibly on 2026-08-19) and scheduled runs — nightly full and the weekly Electron-parity cron — open a rolling ci-health issue on failure or cancellation
- Tripwires no longer misread GitHub API failures: a rate-limited response can no longer fire a false LICENSE-drift alarm (its error body was captured as the "current SHA"), silently skip the CVE/UI/issue-age/digest checks, or auto-clear a genuine stage-batch stall — API errors now fail the job into the self-health handler, and the license check additionally shape-validates the SHA it compares
- Upstream-watch issue titles name only the components that actually drifted (a same-version "webui vX → vX" read as a false update), the daily digest table no longer renders a stray-newline count cell, and a new scheduled-run health tripwire fires a rolling ci-health issue when a watched scheduled workflow (currently the Windows real-Docker smoke) has no recent successful run — the class where an offline self-hosted runner silently erased two months of nightly coverage
