...
**Last updated:** v0.7.6 — Silent failover engine rebuild (#303 symptom 3 — closes #303 fully). Two new substitutions in `fox_overlay/webui_patches/streaming.py` rebuilt on upstream's new `_classify_provider_error()` + `_attempt_credential_self_heal()` architecture. Verification methodology rule unchanged: run against released `:stable` (or candidate `:latest` for pre-tag), not against a local `docker build` from source. The DMG-equivalent install command is the canonical path:
