...
- Releases carry their SBOM again: sbom.yml is now invoked directly from the release pipeline (GitHub never fires `release: published` for workflow-published releases, so every release since v0.7.55 silently shipped without one) and supports tag-targeted backfill; the apt publish job's stale `continue-on-error` was removed now that its secrets are live; the nightly Playwright full suite got a budget sized to today's 57-test suite and a rolling ci-health issue on cron failures (previously 3 shards timed out with the failure surfacing nowhere)
