...
**Last updated:** v0.7.5 — Cleanup + guardrails audit bundle (anchor-drift fail-loud, offline-safe image management, FITB_IMAGE rollback, log rotation, Option B diff guard, `_helpers.py` deduplication). Verification methodology rule unchanged: run against released `:stable` (or candidate `:latest` for pre-tag), not against a local `docker build` from source. The DMG-equivalent install command is the canonical path:
