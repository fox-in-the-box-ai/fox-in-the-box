...
**Last updated:** v0.7.8 — Playwright Phase 1 partial: 4 new integration specs (endpoints sweep + /health deep + static-overlay assets + test-hooks safety) added to Phase 0's /health. Smoke job still non-required; Phase 1 full + becoming-required = v0.7.9+. Verification methodology rule unchanged: run against released `:stable` (or candidate `:latest` for pre-tag), not against a local `docker build` from source. The DMG-equivalent install command is the canonical path:
