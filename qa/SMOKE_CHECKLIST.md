...
**Last updated:** v0.7.9 — Bug-fix pass: #145 workspace persist (p1 data loss), #149 CPU caps for hermes-gateway, #302 bootstrap log surfaced to docker logs. Three independent fixes, one PR, one release. Verification methodology rule unchanged: run against released `:stable` (or candidate `:latest` for pre-tag), not against a local `docker build` from source. The DMG-equivalent install command is the canonical path:
