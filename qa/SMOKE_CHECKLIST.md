...
**Last updated:** v0.7.14 — Process meta-fixes from the v0.7.13 #331 retro: `make validate-overlay` + `validate-overlay.yml` PR gate (catches anchor drift in <30s instead of 3+ min CI build); `make regen-patch` for atomic patch regeneration; `qa/SMOKE_LOG.md` for written audit trail of smoke runs. No user-visible change — pure infrastructure that prevents the #331-class regression from re-shipping. Verification methodology rule unchanged: run against released `:stable` (or candidate `:latest` for pre-tag), not against a local `docker build` from source. The DMG-equivalent install command is the canonical path:
